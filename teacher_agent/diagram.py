...
def _try_gemini_image(prompt, output_path):
    ...
    try:
        ...
        monitor.event('warning', 'Gemini image generation failed; falling back to local renderer: {0}'.format(exc))
        return False
...
def _fallback_linkedin_cover(class_no, title, concepts, output_path, photo_path=None):
    ...
    for radius, alpha, color in [
        ...
        circle = Circle((0.80, 0.50), radius, facecolor='none', edgecolor=color,
                        linewidth=2.0, alpha=(alpha * 0.55 if photo_used else alpha), zorder=3)
        ax.add_patch(circle)
    ...
    for index, x in enumerate(xs):
        ...
        ys.append(0.50 + envelope * math.sin(phase) + 0.018 * math.sin(phase * 2.2))
    ax.plot(xs, ys, color=CYAN, linewidth=1.8, alpha=(0.48 if photo_used else 0.75), zorder=3)
    ax.scatter(xs[::5], ys[::5], s=12, color=BLUE, alpha=(0.42 if photo_used else 0.65), zorder=3)

    ax.text(0.07, 0.84, 'PROFESSOR OS  /  ROBOTICS CLASS {0:02d}'.format(class_no),
            color=CYAN, fontsize=12, fontweight='bold')
    ax.text(0.07, 0.67, title, color=TEXT, fontsize=30, fontweight='bold',
            va='top', wrap=True)
    ax.text(0.07, 0.43,
            'Build the intuition. Test the math. Run the code. Understand the robot.',
            color=MUTED, fontsize=13, wrap=True)

    y = 0.30
    x = 0.07
    for concept in concepts_list[:4]:
        width = min(0.18, 0.055 + 0.010 * len(concept))
        box = FancyBboxPatch((x, y), width, 0.055,
                             boxstyle='round,pad=0.009,rounding_size=0.018',
                             facecolor=PANEL, edgecolor=LINE, linewidth=1.0)
        ax.add_patch(box)
        ax.text(x + width / 2.0, y + 0.0275, concept.title(), ha='center', va='center',
    ...
    fig.savefig(str(output_path), dpi=100, facecolor=BG, bbox_inches=None, pad_inches=0)
    plt.close(fig)
    return output_path
...
def _fallback_teaching_diagram(class_no, title, concepts, output_path):
    ...
    ax.text(0.055, 0.92, 'PROFESSOR OS  /  ENGINEERING VIEW  /  CLASS {0:02d}'.format(class_no),
            fontsize=10.5, weight='bold', color=CYAN, family='sans-serif')
    ax.text(0.055, 0.845, title, fontsize=23, weight='bold', color=TEXT, wrap=True)
    ax.text(0.055, 0.785, 'A technical relationship map for today\'s core ideas', fontsize=11.5, color=MUTED)
    ...
    for index, concept in enumerate(concepts_list):
        ...
        box = FancyBboxPatch((x, y - 0.075), width, 0.15,
                             boxstyle='round,pad=0.008,rounding_size=0.012',
                             facecolor=PANEL, edgecolor=edge, linewidth=1.35)
        ax.add_patch(box)
        ax.text(x + 0.018, y + 0.035, '0{0}'.format(index + 1), color=MUTED,
                fontsize=8.5, weight='bold', ha='left')
        ax.text(x + width / 2.0, y - 0.008, concept.upper(), ha='center', va='center',
                fontsize=10.5, color=TEXT, weight='bold', wrap=True)
        if index < n - 1:
            x1 = x + width + 0.003
            x2 = x + width + gap - 0.003
            ax.annotate('', xy=(x2, y), xytext=(x1, y),
                        arrowprops=dict(arrowstyle='-|>', linewidth=1.4, color=CYAN, alpha=.75))

    ax.text(0.075, 0.66, 'INPUT / OBSERVATION', color=MUTED, fontsize=9, weight='bold')
    ax.plot([0.075, 0.22], [0.64, 0.64], color=CYAN, linewidth=1.2, alpha=.55)
    ax.text(0.925, 0.66, 'OUTPUT / BEHAVIOR', color=MUTED, fontsize=9, weight='bold', ha='right')
    ax.plot([0.78, 0.925], [0.64, 0.64], color=BLUE, linewidth=1.2, alpha=.55)

    ax.text(0.075, 0.245, 'ENGINEERING CHECK', color=CYAN, fontsize=9, weight='bold')
    ax.text(0.075, 0.205,
    ...
    ax.text(0.94, 0.07, 'Built by Connect.Vin', fontsize=9.5, color=MUTED, ha='right')

    fig.savefig(str(output_path), dpi=120, facecolor=BG, bbox_inches=None, pad_inches=0)
    plt.close(fig)
    return output_path
...
def make_linkedin_cover(class_no, title, concepts, output_path, photo_path=None):
    ...
    if _try_gemini_image(_hero_prompt(class_no, title, concepts), output_path):
        return output_path
    return _fallback_linkedin_cover(class_no, title, concepts, output_path, photo_path=photo_path)


def make_teaching_diagram(class_no, title, concepts, output_path):
    output_path = Path(output_path)
    if _try_gemini_image(_diagram_prompt(class_no, title, concepts), output_path):
        return output_path
    return _fallback_teaching_diagram(class_no, title, concepts, output_path)
